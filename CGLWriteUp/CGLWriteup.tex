...
\usepackage{fullpage, setspace, cite, tikz, pgfplots}
...
\begin{center}
\pgfplotstableread{rundata.dat}{\pistonkinetics}
\begin{tikzpicture}[scale=1]
\begin{axis}[width = 1\textwidth, height = 0.5\textwidth, 
	minor tick num=1,title=$Data$ Results,
	xlabel=Time Step, xmin = 0, xmax = 1000, 
	ymin = 0, ylabel =  Age (in time steps alive)]
\addplot [blue, thick] table [x={time}, y={alive}] {\pistonkinetics};
\addplot [green, thick] table [x={time}, y={avgAge}] {\pistonkinetics};
\addplot [red, thick] table [x={time}, y={maxAge}] {\pistonkinetics};
\end{axis}
\end{tikzpicture}
\end{center}
